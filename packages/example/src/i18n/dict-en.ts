import type { zh } from './dict-zh.js';

export const en: typeof zh = {
  'app.brand.tagline': 'Fine-grained DOM framework playground',
  'app.locale.zh': '中文',
  'app.locale.en': 'EN',
  'app.locale.switchHint': 'Switch language here',
  'app.controls.ariaLabel': 'Theme and language settings',
  'app.controls.githubRepo': 'Open kt.js GitHub repository',
  'app.theme.light': 'Light',
  'app.theme.dark': 'Dark',
  'app.theme.switchToLight': 'Switch to light theme',
  'app.theme.switchToDark': 'Switch to dark theme',
  'app.nav.group.features': 'Features',
  'app.nav.group.mui': 'MUI Components',
  'app.pagination.prev': '← Prev',
  'app.pagination.next': 'Next →',
  'app.pagination.noPrev': 'No previous page',
  'app.pagination.noNext': 'No next page',
  'home.discussion.label': 'Discussion',
  'home.discussion.qq': 'QQ Group 1070434849',
  'home.discussion.telegram': 'Telegram Group',
  'home.section.introduction': 'Introduction',
  'home.section.introduction.content':
    'kt.js is a lightweight framework for direct DOM manipulation with <code>jsx</code> support. Unlike React or Vue, which rely on virtual DOM and automatic re-rendering, kt.js gives you fine-grained control over when and how updates happen.',

  'home.section.gettingStarted': 'Getting Started',
  'home.step.createCli': '0. Create with CLI',
  'home.createCli.description':
    'Start with the create command to scaffold a ready-to-run KT.js + Vite + TypeScript project:',
  'home.createCli.more':
    'This generates starter files and base config automatically. You can also use <code>npm create kt.js@latest</code>.',
  'home.gettingStarted.guide': 'Or follow these steps to use KT.js in your project',
  'home.step.installation': '1. Installation',
  'home.step.tsconfig': '2. TypeScript Configuration',
  'home.tsconfig.description': 'Add these settings to your <code>tsconfig.json</code>:',
  'home.step.tsPlugin': '3. TypeScript Plugin',
  'home.tsPlugin.description':
    'Add ts-plugin to your project for better kt.js editor support (mainly for the <code>k-for</code> directive).',
  'home.tsPlugin.vscodeStep':
    'In VSCode, press <code>Ctrl+Shift+P</code>, choose <code>TypeScript: Select TypeScript Version</code>, then choose <code>Use Workspace Version</code>.',
  'home.tsPlugin.restartStep':
    'Finally, select <code>TypeScript: Restart TS Server</code> from the command palette to restart TypeScript.',
  'home.step.viteConfig': '4. Vite Configuration',
  'home.viteConfig.description':
    'Using <code>vite.config.ts</code> is recommended. It supports <code>svg</code> and <code>mathml</code> elements:',
  'home.label.useBabel': 'Use Babel setup',
  'ie11.intro': 'kt.js contains built-in polyfills for certain DOM methods to support IE11.',
  'ie11.description':
    'To make your project fully compatible with IE11, we need to transpile the code with Babel and include necessary polyfills. Below is the Vite configuration used for this purpose:',
  'reactive.section.title': '<code>ref</code> and <code>computed</code>',
  'reactive.section.description':
    '<code>ref()</code> creates a plain <code>KTRef</code>. Read with <code>value</code>; replace the whole outer value with <code>ref.value = nextValue</code>; use <code>ref.draft</code> for deep changes inside nested objects, arrays, <code>Map</code> / <code>Set</code>, or custom mutable objects. <code>computed</code> dependencies still need to be declared manually.',
  'reactive.ref.overview': 'Ref quick guide:',
  'reactive.ref.item.value':
    '<code>value</code> is for reading the current value and replacing the whole outer value: <code>a.value = newValue</code>.',
  'reactive.ref.item.array':
    'Use <code>draft</code> for deep object or array changes, for example <code>a.draft.user.name = "Jane"</code> or <code>a.draft.list.push(item)</code>.',
  'reactive.ref.item.mapSet':
    '<code>Map</code> / <code>Set</code> / custom mutable objects should also be changed through <code>draft</code>, not by mutating inside <code>value</code>.',
  'reactive.ref.item.computed':
    '<code>computed</code> is still read through <code>value</code>; it stays read-only and has no <code>draft</code>.',
  'reactive.ref.item.weak':
    'Weak containers are wrapped too, which is useful when state is keyed only by object identity.',
  'reactive.ref.item.date':
    'Calling setters such as <code>setFullYear</code> or <code>setUTCFullYear</code> emits changes automatically.',
  'reactive.attribute.title': 'Attribute Ref',
  'reactive.attribute.description': 'Make attributes reactive.',
  'reactive.attribute.currentWidth': 'Current Width: {{1}}',
  'reactive.children.title': 'Children',
  'reactive.children.description': 'Make children reactive.',
  'reactive.children.initial': 'Initial Child',
  'reactive.children.updated': 'Updated Child',
  'reactive.api.title': 'KTReactive Public Methods',
  'reactive.api.description':
    'The following blocks explain each <code>KTReactive</code> public method in practice: for <code>ref</code>, focus on <code>value</code> / <code>draft</code>; <code>computed</code> is still read through <code>value</code> and stays read-only.',
  'reactive.api.overview': 'Core API overview:',
  'reactive.api.method.value':
    'Read the current value, and also replace the whole outer value with <code>ref.value = nextValue</code>.',
  'reactive.api.method.draft':
    'Use it for deep reactive mutations. You may change nested content, but you must not assign to it directly. <code>ref.draft.user.name = "Jane"</code> is valid; <code>ref.draft = nextValue</code> is not.',
  'reactive.api.method.notify': 'Force listeners to run.',
  'reactive.api.method.map': 'Derive a <code>computed</code> from current reactive with optional extra dependencies.',
  'reactive.api.method.get':
    'Read a nested path from the current reactive and get a lightweight read-only <code>KTSubComputed</code>.',
  'reactive.api.method.subref':
    'Get a writable nested field from a <code>ref</code>; updating the subref also updates the parent ref.',
  'reactive.api.method.is':
    'Build a boolean <code>KTSubComputed</code> that checks equality with <code>Object.is</code> semantics.',
  'reactive.api.method.match':
    'Build a boolean <code>KTSubComputed</code> that performs deep object/array pattern matching.',
  'reactive.api.method.addOnChange': 'Register a value-change listener with optional custom key.',
  'reactive.api.method.removeOnChange': 'Remove a listener by key.',
  'reactive.api.value.title': '<code>value</code>: read + whole-value replace',
  'reactive.api.value.description':
    'Use it to read the current value, or to replace the whole outer value with <code>ref.value = nextValue</code>. Deep mutations such as <code>ref.value.user.name = "Jane"</code> are not tracked through <code>value</code>.',
  'reactive.api.value.current': 'Current value: {{1}}',
  'reactive.api.value.double': 'Double result (map): {{1}}',
  'reactive.api.value.decrement': '-1',
  'reactive.api.value.increment': '+1',
  'reactive.api.draft.title': '<code>draft</code>: deep reactive mutation',
  'reactive.api.draft.description':
    'Nested objects, arrays, <code>Map</code> / <code>Set</code>, and custom mutable objects should be changed through <code>draft</code>. Mutate inner fields or call mutating methods; do not write <code>ref.draft = nextValue</code>.',
  'reactive.api.draft.summary': 'Name: {{1}}; Visits: {{2}}; Tags: {{3}}',
  'reactive.api.draft.replace': 'Replace name by value',
  'reactive.api.draft.bumpVisits': 'Visits +1',
  'reactive.api.draft.addTag': 'Add tag',
  'reactive.api.draft.removeTag': 'Remove last tag',
  'reactive.api.map.title': '<code>map</code>',
  'reactive.api.map.description':
    'Derive a computed value from the current value and declare extra dependencies as the second argument (e.g. discount and tax).',
  'reactive.api.map.price': 'Base price: {{1}}',
  'reactive.api.map.discount': 'Discount: {{1}}%',
  'reactive.api.map.tax': 'Tax: {{1}}%',
  'reactive.api.map.total': 'Final price: {{1}}',
  'reactive.api.map.priceMinus': 'Price -10',
  'reactive.api.map.pricePlus': 'Price +10',
  'reactive.api.map.discountMinus': 'Discount -5%',
  'reactive.api.map.discountPlus': 'Discount +5%',
  'reactive.api.map.taxMinus': 'Tax -1%',
  'reactive.api.map.taxPlus': 'Tax +1%',
  'reactive.api.get.title': '<code>get</code>: read nested paths',
  'reactive.api.get.description':
    '<code>get(...keys)</code> is useful when you want to expose a deep field directly. It stays in sync with the parent reactive while remaining read-only.',
  'reactive.api.get.cityLabel': 'City',
  'reactive.api.get.planLabel': 'Plan',
  'reactive.api.get.ordersLabel': 'Orders',
  'reactive.api.get.rotateCity': 'Rotate City',
  'reactive.api.get.upgradePlan': 'Upgrade Plan',
  'reactive.api.get.addOrder': 'Orders +1',
  'reactive.api.subref.title': '<code>subref</code>: write nested paths',
  'reactive.api.subref.description':
    '<code>subref(...keys)</code> returns a writable nested ref. Assigning to its <code>value</code> keeps the parent ref in sync and emits updates.',
  'reactive.api.subref.summary': 'Title: {{1}}; Theme: {{2}}; Volume: {{3}}',
  'reactive.api.subref.titleLabel': 'Sub title',
  'reactive.api.subref.themeLabel': 'Sub theme',
  'reactive.api.subref.volumeLabel': 'Sub volume',
  'reactive.api.subref.rename': 'Rename Title',
  'reactive.api.subref.toggleTheme': 'Toggle Theme',
  'reactive.api.subref.volumeUp': 'Volume +5',
  'reactive.api.subref.volumeDown': 'Volume -5',
  'reactive.api.listeners.title': '<code>addOnChange</code> / <code>removeOnChange</code>',
  'reactive.api.listeners.description':
    'Manage listener lifecycle via key; once removed, no further updates are received.',
  'reactive.api.listeners.enable': 'Enable listener',
  'reactive.api.listeners.disable': 'Disable listener',
  'reactive.api.listeners.bump': 'Trigger +1',
  'reactive.api.listeners.clear': 'Clear logs',
  'reactive.api.listeners.status': 'Listener status: {{1}}',
  'reactive.api.listeners.status.on': 'ON',
  'reactive.api.listeners.status.off': 'OFF',
  'reactive.api.listeners.value': 'Observed value: {{1}}',
  'reactive.api.listeners.log.empty': 'No logs yet.',
  'reactive.api.listeners.log.listenerOn': 'Listener registered.',
  'reactive.api.listeners.log.listenerOff': 'Listener removed.',
  'reactive.api.listeners.log.changed': '[{{3}}] Value changed: {{1}} -> {{2}}',
  'reactive.isMatch.section.title': '<code>is</code> and <code>match</code>',
  'reactive.isMatch.section.description':
    '<code>is</code> creates a boolean <code>KTSubComputed</code> for equality checks, and <code>match</code> performs deep object/array pattern matching.',
  'reactive.isMatch.playground.title': '<code>is</code> + <code>match</code>',
  'reactive.isMatch.playground.description':
    'Use boolean computed values for state checks; reactive targets and matchers are supported.',
  'reactive.isMatch.button.targetPlus': 'Target +1000',
  'reactive.isMatch.button.syncMatcher': 'Sync matcher to theme',
  'reactive.isMatch.button.toggleMatcher': 'Toggle matcher theme',
  'reactive.isMatch.label.target': 'Salary target',
  'reactive.isMatch.label.is': 'salary.is(target)',
  'reactive.isMatch.label.matcher': 'Matcher',
  'reactive.demo.section.data.title': 'Reactive Data',
  'reactive.demo.section.data.description':
    '<code>ref</code> and <code>computed</code> are used to build reactive state and derived values.',
  'reactive.demo.section.subpath.title': 'Reactive Sub-properties',
  'reactive.demo.section.subpath.description':
    'Use <code>get</code> and <code>subref</code> to expose lightweight reactive sub-properties.',
  'reactive.demo.section.map.title': 'Derived <code>computed</code>',
  'reactive.demo.section.map.description': 'Use <code>map</code> to derive <code>computed</code> values.',
  'reactive.demo.section.deep.title': 'Deep Reactivity',
  'reactive.demo.section.deep.description':
    '<code>ref.value</code> only tracks outer replacement. For deep reactive updates, use <code>ref.draft</code>; it marks the ref and schedules updates in a microtask for efficient deep tracking.',
  'reactive.demo.section.deep.extra': 'Complex custom mutable objects are also supported.',
  'reactive.demo.section.playground.title': 'Live Combined Playground',
  'reactive.demo.section.playground.description':
    'This is a real running playground that combines the capabilities on this page with immediate linked updates.',
  'reactive.demo.panel.refComputed.title': '<code>ref</code> + <code>computed</code>',
  'reactive.demo.panel.refComputed.description':
    'Both base state and derived values are driven by real reactive objects.',
  'reactive.demo.panel.refComputed.button.salaryPlus': 'Salary +1000',
  'reactive.demo.panel.refComputed.button.bonusPlus': 'Bonus rate +1%',
  'reactive.demo.panel.refComputed.button.reset': 'Reset',
  'reactive.demo.panel.refComputed.label.baseSalary': 'Base salary',
  'reactive.demo.panel.refComputed.label.bonusRate': 'Bonus rate',
  'reactive.demo.panel.refComputed.label.annualBonus': 'Annual bonus',
  'reactive.demo.panel.refComputed.label.totalIncome': 'Total income',
  'reactive.demo.panel.getSubref.title': '<code>get</code> + <code>subref</code>',
  'reactive.demo.panel.getSubref.description':
    '<code>get</code> gives a read-only view, while <code>subref</code> splits nested paths into writable pieces.',
  'reactive.demo.panel.getSubref.inputAria': 'subref name input',
  'reactive.demo.panel.getSubref.button.toggleTheme': 'Toggle theme',
  'reactive.demo.panel.getSubref.button.volumeDown': 'Volume -5',
  'reactive.demo.panel.getSubref.button.volumeUp': 'Volume +5',
  'reactive.demo.panel.getSubref.label.getName': "get('name')",
  'reactive.demo.panel.getSubref.label.themeSubref': 'theme subref',
  'reactive.demo.panel.getSubref.label.volumeSubref': 'volume subref',
  'reactive.demo.panel.map.title': '<code>map</code>',
  'reactive.demo.panel.map.description':
    'Derived computed values refresh automatically when the source ref or sub-path changes.',
  'reactive.demo.panel.map.label.salaryBand': 'Salary band',
  'reactive.demo.panel.map.label.profileBadge': 'Profile badge',
  'reactive.demo.panel.draft.title': '<code>draft</code> + Complex Objects',
  'reactive.demo.panel.draft.description':
    'Deep objects, arrays, Map, and custom mutable objects are updated via <code>draft</code>.',
  'reactive.demo.panel.draft.button.visitsPlus': 'Visits +1',
  'reactive.demo.panel.draft.button.addTag': 'Add tag',
  'reactive.demo.panel.draft.button.raiseMary': 'Mary salary +500',
  'reactive.demo.panel.draft.button.badgePulse': 'badge pulse',
  'reactive.demo.panel.draft.button.badgeRename': 'badge rename',
  'reactive.demo.panel.draft.label.visits': 'Visits',
  'reactive.demo.panel.draft.label.tags': 'Tags',
  'reactive.demo.panel.draft.label.marySalary': 'Mary salary from Map',
  'reactive.demo.panel.draft.label.customObject': 'Custom object',
  'reactive.demo.label.volume': 'volume',
  'reactive.demo.band.senior': 'senior band',
  'reactive.demo.band.growth': 'growth band',
  'reactive.demo.bool.true': 'true',
  'reactive.demo.bool.false': 'false',
  'events.intro':
    'KT.js uses the <code>on:</code> syntax for event handling, similar to Svelte, with direct DOM events.',
  'events.output.none': 'No events yet.',
  'events.output.clickedAt': '✓ Clicked at {{1}}',
  'events.output.doubleClicked': '✓ Double clicked!',
  'events.output.mouseEntered': '✓ Mouse entered!',
  'events.output.mouseLeft': '✓ Mouse left!',
  'events.output.focused': '✓ Button focused!',
  'events.output.blurred': '✓ Button blurred!',
  'events.button.click': 'Click Event',
  'events.button.doubleClick': 'Double Click',
  'events.button.hover': 'Hover Me',
  'events.button.focus': 'Focus Me',
  'events.counter.title': 'Counter',
  'events.counter.description': 'A simple counter demonstrating manual state updates in KT.js.',
  'events.counter.label': 'Count:',
  'events.counter.reset': 'Reset',
  'directives.title': 'Directives',
  'directives.ifElse.title': 'k-if + k-else',
  'directives.ifElse.description':
    '<code>k-if</code> and <code>k-else</code> can be used as adjacent sibling branches.',
  'directives.ifElse.showIf': 'Show k-if',
  'directives.ifElse.showElse': 'Show k-else',
  'directives.ifElse.toggle': 'Toggle',
  'directives.ifElse.condition': 'Condition:',
  'directives.ifElse.state.true': 'true',
  'directives.ifElse.state.false': 'false',
  'directives.ifElse.branchIf': 'k-if branch: primary content is rendered.',
  'directives.ifElse.branchElse': 'k-else branch: fallback content is rendered.',
  'directives.ifElse.syntax': 'Syntax rule: k-else must be immediately after a sibling k-if.',
  'directives.model.title': 'k-model',
  'directives.model.description': 'k-model must pass a ref to implement two-way binding.',
  'directives.model.initialValue': 'Reactive text',
  'directives.model.updatedAt': 'Updated at: {{1}}',
  'directives.html.title': 'k-html',
  'directives.html.initialContent': 'Initial HTML',
  'directives.html.description': 'k-html accepts a ref and redraws automatically when the ref changes.',
  'directives.fullDemo': 'Full Demo',
  'otherElements.svgTitle': 'SVG',
  'otherElements.mathmlTitle': 'MathML',
  'fragment.default.header': 'Header',
  'fragment.default.content': 'Content',
  'fragment.default.actions': 'Actions',
  'fragment.reactive.sectionPrefix': 'Section',
  'fragment.shorthand.title': 'Fragment Shorthand',
  'fragment.shorthand.description':
    'Use <code>&lt;&gt;...&lt;/&gt;</code> to return multiple siblings without adding an extra wrapper element.',
  'fragment.shorthand.preview.title': 'Title',
  'fragment.shorthand.preview.description': 'Description',
  'fragment.shorthand.preview.actions': 'Actions',
  'fragment.shorthand.note': 'These pills are direct siblings in the container, so flex/grid layouts stay flat.',
  'fragment.reactive.title': 'Reactive Fragment',
  'fragment.reactive.description':
    'Combine <code>Fragment</code> with a <code>ref</code> array to redraw sibling nodes in-place whenever the list changes.',
  'fragment.reactive.addItem': 'Add Item',
  'fragment.reactive.removeLast': 'Remove Last',
  'fragment.reactive.rotate': 'Rotate',
  'fragment.reactive.reset': 'Reset',
  'fragment.reactive.summary': '{{1}} nodes: {{2}}',
  'nav.home.label': 'Getting Started',
  'nav.home.title': 'Welcome to KT.js Framework',
  'nav.home.description': 'Introduction to KT.js framework and core concepts',
  'nav.ie11.label': 'IE11 Compatibility',
  'nav.ie11.title': 'How to Configure for IE11 Support',
  'nav.ie11.description': 'We need babel to downgrade the code and include necessary polyfills.',
  'nav.changelog.label': 'ChangeLog',
  'nav.changelog.title': 'View ChangeLog',
  'nav.changelog.description': 'A comprehensive list of changes, updates, and improvements in the KT.js framework',
  'nav.reactive.label': 'Reactive',
  'nav.reactive.title': 'Reactive',
  'nav.reactive.description': 'Demonstration of reactive features using kt.js refs',
  'nav.directives.label': 'Directives',
  'nav.directives.title': 'Directives',
  'nav.directives.description': 'Examples for k-if/k-else, k-model, and k-html directives',
  'nav.fragment.label': 'Fragment',
  'nav.fragment.title': 'Fragment',
  'nav.fragment.description': 'Demonstrates fragment shorthand and reactive fragment children arrays',
  'nav.events.label': 'Events',
  'nav.events.title': 'Events Handling',
  'nav.events.description':
    'KT.js uses the on: syntax for event handling, similar to React but with direct DOM events.',
  'nav.otherElements.label': 'Other Elements',
  'nav.otherElements.title': 'Non-HTMLElement Elements',
  'nav.otherElements.description':
    'KT.js supports creating non-HTMLElement elements like svg and mathml using jsx syntax. This demo shows how to use them.',
  'nav.trivia.label': 'Trivia',
  'nav.trivia.title': 'Trivia',
  'nav.trivia.description': 'Some interesting facts and lesser-known features about KT.js framework',

  'nav.mui.button.label': 'Button',
  'nav.mui.button.title': 'MUI Button',
  'nav.mui.button.description': 'Material-UI Button component',
  'nav.mui.alert.label': 'Alert',
  'nav.mui.alert.title': 'MUI Alert',
  'nav.mui.alert.description': 'Material-UI Alert component',
  'nav.mui.dropdownButton.label': 'DropdownButton',
  'nav.mui.dropdownButton.title': 'MUI DropdownButton',
  'nav.mui.dropdownButton.description': 'Material-UI DropdownButton component',
  'nav.mui.tabs.label': 'Tabs',
  'nav.mui.tabs.title': 'MUI Tabs',
  'nav.mui.tabs.description': 'Material-UI Tabs component',
  'nav.mui.textfield.label': 'TextField',
  'nav.mui.textfield.title': 'MUI TextField',
  'nav.mui.textfield.description': 'Material-UI TextField component',
  'nav.mui.filepicker.label': 'FilePicker',
  'nav.mui.filepicker.title': 'MUI FilePicker / DirectoryPicker',
  'nav.mui.filepicker.description': 'Material-UI file and directory picker components',
  'mui.filepicker.description': 'Pick files or a whole directory with native inputs and KT refs.',
  'mui.filepicker.file.description':
    'Supports multiple files, file type filters, helper text, and <code>k-model</code>.',
  'mui.filepicker.file.label': 'Project assets',
  'mui.filepicker.file.placeholder': 'Choose images or markdown files',
  'mui.filepicker.file.helper': 'Try selecting several files to see the summary update.',
  'mui.filepicker.file.button': 'Browse',
  'mui.filepicker.file.clear': 'Clear Files',
  'mui.filepicker.file.empty': 'No files selected yet.',
  'mui.filepicker.file.summary': '{{1}} file(s): {{2}}',
  'mui.filepicker.directory.description':
    'Uses browser directory selection and exposes the top-level folder plus file list.',
  'mui.filepicker.directory.label': 'Workspace folder',
  'mui.filepicker.directory.placeholder': 'Choose a directory',
  'mui.filepicker.directory.helper': 'Directory selection depends on browser support for webkitdirectory.',
  'mui.filepicker.directory.button': 'Select',
  'mui.filepicker.directory.clear': 'Clear Directory',
  'mui.filepicker.directory.empty': 'No directory selected yet.',
  'mui.filepicker.directory.selected': 'Selected directory',
  'mui.filepicker.directory.summary': '{{1}} · {{2}} file(s){{3}}',
  'nav.mui.textfieldRef.label': 'TextField (Reactive Refs)',
  'nav.mui.textfieldRef.title': 'MUI TextField with Reactive Refs',
  'nav.mui.textfieldRef.description': 'Demonstrates all TextField properties controlled via KTRefs',
  'nav.mui.dialog.label': 'Dialog',
  'nav.mui.dialog.title': 'MUI Dialog',
  'nav.mui.dialog.description': 'Material-UI Dialog component',
  'nav.mui.progress.label': 'Progress',
  'nav.mui.progress.title': 'MUI LinearProgress',
  'nav.mui.progress.description': 'Material-UI Progress indicator',
  'nav.mui.radio.label': 'Radio',
  'nav.mui.radio.title': 'MUI Radio',
  'nav.mui.radio.description': 'Material-UI Radio component',
  'nav.mui.checkbox.label': 'Checkbox',
  'nav.mui.checkbox.title': 'MUI Checkbox',
  'nav.mui.checkbox.description': 'Material-UI Checkbox component',
  'nav.mui.switch.label': 'Switch',
  'nav.mui.switch.title': 'MUI Switch',
  'nav.mui.switch.description': 'Material-UI Switch component',
  'nav.mui.select.label': 'Select',
  'nav.mui.select.title': 'MUI Select',
  'nav.mui.select.description': 'Material-UI Select component',
  'nav.mui.menu.label': 'Menu',
  'nav.mui.menu.title': 'MUI Menu',
  'nav.mui.menu.description': 'Material-UI Menu component',
  'nav.mui.formlabel.label': 'FormLabel',
  'nav.mui.formlabel.title': 'MUI FormLabel',
  'nav.mui.formlabel.description': 'Material-UI FormLabel component',
  'nav.mui.card.label': 'Card',
  'nav.mui.card.title': 'MUI Card',
  'nav.mui.card.description': 'Material-UI Card component',
  'nav.mui.pill.label': 'Pill',
  'nav.mui.pill.title': 'MUI Pill',
  'nav.mui.pill.description': 'Material-UI Pill component',
  'nav.mui.badge.label': 'Badge',
  'nav.mui.badge.title': 'MUI Badge',
  'nav.mui.badge.description': 'Material-UI Badge component',
  'nav.mui.popover.label': 'Popover',
  'nav.mui.popover.title': 'MUI Popover',
  'nav.mui.popover.description': 'Material-UI Popover component',
  'nav.mui.bottomNavigation.label': 'BottomNavigation',
  'nav.mui.bottomNavigation.title': 'MUI BottomNavigation',
  'nav.mui.bottomNavigation.description': 'Material-UI BottomNavigation component',
  'nav.mui.modal.label': 'Modal Helpers',
  'nav.mui.modal.title': 'MUI Modal Helpers',
  'nav.mui.modal.description': 'Dialog-based replacements for alert / confirm / prompt',
  'nav.mui.icons.label': 'Icons',
  'nav.mui.icons.title': 'MUI Icons',
  'nav.mui.icons.description': 'Material-UI Icons',
};
